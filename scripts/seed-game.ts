...
import { gameQuestions, defaultGameConfig } from "./game-data";
...
async function main() {
  ...
  if (isNaN(startMinutes) || isNaN(durationHours)) {
    ...
    console.error("Example: pnpm seed:game 5 2  (starts in 5 minutes, lasts 2 hours)");
    process.exit(1);
  }
  ...
  for (const round of gameQuestions) {
    ...
    console.log(`✓ Created round ${round.roundNum} with ${round.data.length} questions`);
  }

  console.log(`\n✅ Successfully seeded game ${game.id}!`);
  console.log(`   Total rounds: ${gameQuestions.length}`);
  console.log(`   Total questions: ${gameQuestions.reduce((sum, r) => sum + r.data.length, 0)}`);
}
