...
export default function BuyWafflePage() {
  ...
  const { game, loadActiveGame } = useGame();
  ...
  useEffect(() => {
    if (!farcasterId || !game?.id) return;
    fetchTicket(farcasterId, game.id).catch((error) =>
      console.error("Failed to fetch ticket info", error)
    );
  }, [farcasterId, game?.id, fetchTicket]);

  useEffect(() => {
    if (!farcasterId || !game?.id) {
      setFriends([]);
    ...
    const loadFriends = async () => {
      try {
        ...
        const res = await fetch(
          `/api/social/friends?fid=${farcasterId}&gameId=${game?.id}`,
